...
"""walker2d_learner — auto-generated SKELETON sync-gate plugin.
...
nothing with actions or state. To wire it up:
  ...
  1. Decide on action mapping (cmd_force / cmd_vel / cmd_pos) for each
     ...
     gazebo_gymnasium_resources/models/walker2d/walker2d.sdf for joint
     names and the actuator plugin attached.
  2. Decide observation vector (look at canonical Gymnasium
     `gymnasium/envs/mujoco/walker2d_v5.py` for the reward + obs layout).
...
class Walker2dSyncGate:
    ...
    def configure(self, entity, element, ecm, eventManager):
        ...
        self.agent_name = element.get_string("agent_name") or "walker2d"
        ...
        print(f"[Walker2dSyncGate] LOADED but UNIMPLEMENTED for env={self.agent_name!r}")
        print(f"[Walker2dSyncGate] Actions arriving on /env/action will be ignored.")
        print(f"[Walker2dSyncGate] Implement pre_update/post_update + topic callbacks "
    ...
    def _on_action(self, msg):
        if not self._has_warned:
            print(f"[Walker2dSyncGate] received action — but plugin is UNIMPLEMENTED.")
            self._has_warned = True


def get_system():
    return Walker2dSyncGate()
