...
"""pusher_v5_learner — auto-generated SKELETON sync-gate plugin.
...
nothing with actions or state. To wire it up:
  ...
  1. Decide on action mapping (cmd_force / cmd_vel / cmd_pos) for each
     ...
     gazebo_gymnasium_resources/models/pusher_v5/pusher_v5.sdf for joint
     names and the actuator plugin attached.
  2. Decide observation vector (look at canonical Gymnasium
     `gymnasium/envs/mujoco/pusher_v5_v5.py` for the reward + obs layout).
...
class PusherV5SyncGate:
    ...
    def configure(self, entity, element, ecm, eventManager):
        ...
        self.agent_name = element.get_string("agent_name") or "pusher_v5"
        ...
        print(f"[PusherV5SyncGate] LOADED but UNIMPLEMENTED for env={self.agent_name!r}")
        print(f"[PusherV5SyncGate] Actions arriving on /env/action will be ignored.")
        print(f"[PusherV5SyncGate] Implement pre_update/post_update + topic callbacks "
    ...
    def _on_action(self, msg):
        if not self._has_warned:
            print(f"[PusherV5SyncGate] received action — but plugin is UNIMPLEMENTED.")
            self._has_warned = True


def get_system():
    return PusherV5SyncGate()
