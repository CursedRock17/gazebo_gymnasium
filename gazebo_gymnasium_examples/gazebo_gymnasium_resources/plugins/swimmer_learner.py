...
"""swimmer_learner — auto-generated SKELETON sync-gate plugin.
...
nothing with actions or state. To wire it up:
  ...
  1. Decide on action mapping (cmd_force / cmd_vel / cmd_pos) for each
     ...
     gazebo_gymnasium_resources/models/swimmer/swimmer.sdf for joint
     names and the actuator plugin attached.
  2. Decide observation vector (look at canonical Gymnasium
     `gymnasium/envs/mujoco/swimmer_v5.py` for the reward + obs layout).
...
class SwimmerSyncGate:
    ...
    def configure(self, entity, element, ecm, eventManager):
        ...
        self.agent_name = element.get_string("agent_name") or "swimmer"
        ...
        print(f"[SwimmerSyncGate] LOADED but UNIMPLEMENTED for env={self.agent_name!r}")
        print(f"[SwimmerSyncGate] Actions arriving on /env/action will be ignored.")
        print(f"[SwimmerSyncGate] Implement pre_update/post_update + topic callbacks "
    ...
    def _on_action(self, msg):
        if not self._has_warned:
            print(f"[SwimmerSyncGate] received action — but plugin is UNIMPLEMENTED.")
            self._has_warned = True


def get_system():
    return SwimmerSyncGate()
