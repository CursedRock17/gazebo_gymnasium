...
class CartPole(gazebo_single_agent.GazeboSingleAgent):
    def __init__(self):
        ...
        self.cmd_vel_node = Node()
        cmd_vel_topic = "/model" + self.agent_name + "/joint/slider_to_cart/0/cmd_pos"
        cmd_vel_opts = AdvertiseMessageOptions()
        self.cmd_vel_pub = self.cmd_vel_node.advertise(cmd_vel_topic, Double, cmd_vel_opts)
    ...
    def apply_action(self, action):
        ...
        if real_action == 0:
            ...
            doubleMsg = Double()
            doubleMsg.data = -2.5
            self.cmd_vel_pub.publish(doubleMsg)
        elif real_action == 1:
            # Move Right
            doubleMsg = Double()
            doubleMsg.data = 2.5
    ...
    def is_terminated(self) -> bool:
        ...
        # fail (leave range) or suceed (keep pole up long enough)
        if self.episode_reward >= self.reward_goal:
            # We have survived 500 Steps
            print("Succeed Episode: ", self.current_episode)
            return True
