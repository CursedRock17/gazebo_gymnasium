...
class DiffDriveLearner(gazebo_single_agent.GazeboSingleAgent):
    ...
    def __init__(self):
        ...
        self.observation_space = Box(
            low=np.array([self.min_linear_x, self.min_angular_z, self.min_pos_x,
                         self.min_pos_y, self.min_yaw, self.min_goal_pos_x, self.min_goal_pos_y]),
            high=np.array([self.max_linear_x, self.max_angular_z, self.max_pos_x,
                          self.max_pos_y, self.max_yaw, max_goal_pos_x, max_goal_pos_y]),
            shape=(7,),
            dtype=np.float64
        )
    ...
    def get_observation(self):
        ...
        self.rover_linear_velocity_x = utils.normalize_to_range(
            self.rover_linear_velocity_x,
            self.min_linear_x, self.max_linear_x, -1.0, 1.0, clip=True)
        self.rover_angular_velocity_z = utils.normalize_to_range(
            self.rover_angular_velocity_z,
            self.min_angular_z, self.max_angular_z, -1.0, 1.0, clip=True)
    ...
    def set_default_observation(self):
        # Return the entire space as zeros
