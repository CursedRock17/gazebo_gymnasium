#!/usr/bin/env python3
"""Train Ant in Gazebo with PPO (Stable-Baselines3).
...
class EpisodeLogger(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals["dones"][0]:
            self._ep_count += 1
            print(f"  ep {self._ep_count:4d} | steps={self._ep_len:4d} | "
                  f"reward={self._ep_reward:.1f} | total={self.num_timesteps}")
            self._ep_reward = 0.0
            self._ep_len = 0
        return True
...
def parse_args():
    ...
    p.add_argument("--save-path", default="ant_ppo")
    p.add_argument("--tensorboard-log", default="./tb_logs_ant")
