...
1. **Lower IPC overhead** between nodes in the same container — intra-process
   message passing skips DDS serialization (saves ~50–200 µs per message).
2. **Fewer process startups** — one container vs N node executables.
   Saves ~50–100 ms of startup time per saved node.
...
So 3–4 ROS nodes max per launch. Modest.
...
1. **Bottleneck math.** Our RL training spends ≥99% of wall time inside
   gz sim physics, the agent's policy forward pass, and the agent's
   gradient update. The bridges pass perhaps 50–100 messages/second
   (joint_state, pose, /env/metrics). Even if composing saves 200 µs
   per message, that's 10–20 ms/sec — well under 1% of wall time.

2. **Simpler launches.** Standalone `Node()` is one line per node; a
   composable container needs a `ComposableNodeContainer` + per-node
...
- You have ≥5 nodes that all talk to each other a lot.
