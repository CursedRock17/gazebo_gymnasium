...
def cartpole_multi(monkeypatch):
    ...
    monkeypatch.setattr(world_control.WorldController, "_send", lambda self, *a, **kw: True)

    from gazebo_gymnasium_bridge.envs import make_multi

    env = make_multi("cartpole", n_agents=3, world_name="t", reset_timeout=0.01, step_timeout=0.01)
...
class TestVecEnvContract:
    ...
    def test_group_reset_when_all_fallen(self, cartpole_multi):
        ...
        cartpole_multi._latest_states[:] = np.array([0.0, 0.0, 0.5, 0.0], dtype=np.float32)
        cartpole_multi.step_async(np.zeros(3, dtype=int))
...
class TestSB3:
    def test_ppo_accepts_generalized_env(self, cartpole_multi):
        ...
        model = sb3.PPO(
            "MlpPolicy", cartpole_multi, n_steps=4, batch_size=4, n_epochs=1, verbose=0
        )
...
class TestBackwardCompat:
    def test_multicartpole_is_generalized(self, monkeypatch):
        ...
        monkeypatch.setattr(world_control.WorldController, "_send", lambda self, *a, **kw: True)
        from gazebo_gymnasium_bridge.envs import MultiAgentGazeboVecEnv
