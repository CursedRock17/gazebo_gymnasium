...
class PPOAgent:
    ...
    def __init__(self, number_of_inputs, number_of_actor_outputs, clip_param=0.2, max_grad_norm=0.5, ppo_update_iters=5,
                 batch_size=8, gamma=0.99, actor_lr=0.001, critic_lr=0.003, seed=None):
        super().__init__()
        if seed is not None:
            manual_seed(seed)
    ...
    def work(self, agent_input, type_="simple"):
        """
        type_ == "simple"
            Implementation for a simple forward pass for all box values.
        type == "discrete"
            Implementation for a simple forward pass for probability between 2 values
        type_ == "maxiumum"
            Implementation for the forward pass, that returns the max selected value.
        """
